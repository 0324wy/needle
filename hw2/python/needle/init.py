...
def xavier_uniform(fan_in, fan_out, gain=1.0, **kwargs):
    ...
    return rand(fan_in, fan_out, low=-a, high=a, requires_grad=True)


def xavier_normal(fan_in, fan_out, gain=1.0, **kwargs):
    a = gain * math.sqrt(2 / (fan_in + fan_out))
    return randn(fan_in, fan_out, mean=0, std=a, requires_grad=True)
...
def kaiming_uniform(fan_in, fan_out, nonlinearity="relu", **kwargs):
    ...
    if fan_out is None:
        return rand(fan_in, low=-a, high=a, requires_grad=True)
    else:
        return rand(fan_in, fan_out, low=-a, high=a, requires_grad=True)


def kaiming_normal(fan_in, fan_out, gain=math.sqrt(2), **kwargs):
