...
class Tensor(Value):
    ...
    def backward(self, out_grad=None):
        ...
        compute_gradient_of_variables2(self, out_grad)
...
def compute_gradient_of_variables2(output_tensor, out_grad):
    ...
    for node in reverse_topo_order:
        ...
        adjoint = 0
        for _, partial_adjoint in enumerate(partial_adjoint_list):
            adjoint = adjoint + partial_adjoint
        node.grad = adjoint
        # print(node.grad)

        # 计算指向该节点的partial adjoint
        if len(node.inputs) == 0:
            continue
        # print('adjoint-----', node.grad.shape)
        # print('node-----', node.shape)
        out_grad = node.op.gradient(node.grad, node)
        # print('nodeInputs', len(node.inputs))
        if node_to_output_grads_list.get(node.inputs[0]) is None:
            node_to_output_grads_list[node.inputs[0]] = []

        # print('node_to_output_grads_list', len(node_to_output_grads_list))
        if isinstance(out_grad, tuple):
            if len(node.inputs) == 2:
                if node_to_output_grads_list.get(node.inputs[1]) is None:
                    node_to_output_grads_list[node.inputs[1]] = []
                node_to_output_grads_list[node.inputs[1]].append(out_grad[1])
            node_to_output_grads_list[node.inputs[0]].append(out_grad[0])
        else:
